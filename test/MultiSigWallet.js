...
contract('MultiSigWallet', (accounts) => {
  ...
  describe('construction', async () => {
    ...
    context('success', async () => {
      ...
      it('should be initialized with 0 balance', async () => {
        ...
        expect(web3.eth.getBalance(wallet.address).toNumber()).to.eql(0);
      });

      it('should initialize owners', async () => {
        const wallet = await MultiSigWalletMock.new(owners, requirement);

        expect(owners).to.have.members(await wallet.getOwners());
      });
      ...
      it('should initialize requirement', async () => {
        ...
        expect(requirement).to.eql((await wallet.required()).toNumber());
      });

      it('should initialize with empty transaction count', async () => {
        const wallet = await MultiSigWalletMock.new(owners, requirement);

        expect((await wallet.transactionCount()).toNumber()).to.eql(0);
      });
    });
  });
  ...
  describe('fallback function', async () => {
    ...
    it('should receive ETH', async () => {
      ...
      expect(walletBalance.toNumber()).to.eql(0);
      ...
      expect(walletBalance2.toNumber()).to.eql(walletBalance.plus(value).toNumber());
    });
    ...
    it('should receive ERC20', async () => {
      ...
      expect(senderBalance.toNumber()).to.eql(value);
      expect(walletBalance.toNumber()).to.eql(0);

      await token.transfer(wallet.address, value, { from: sender });

      const senderBalance2 = await token.balanceOf(sender);
      expect(senderBalance2.toNumber()).to.eql(senderBalance.minus(value).toNumber());

      const walletBalance2 = await token.balanceOf(wallet.address);
      expect(walletBalance2.toNumber()).to.eql(walletBalance.plus(value).toNumber());
    });
  });
    ...
    [
      ...
      context(`with ${spec.owners.length} owners and requirement of ${spec.requirement}`, async () => {
        ...
        beforeEach(async () => {
          ...
          expect(web3.eth.getBalance(wallet.address).toNumber()).to.eql(initETHBalance);

          token = await TestERC20Token.new(initERC20Balance);

          await token.transfer(wallet.address, initERC20Balance);

          expect((await token.balanceOf(wallet.address)).toNumber()).to.eql(initERC20Balance);
          expect((await token.balanceOf(receiver)).toNumber()).to.eql(0);
        });
        ...
        describe('revokeConfirmation', async () => {
          ...
          if (spec.requirement > 2) {
            it('should revoke a confirmation', async () => {
              ...
              const confirmer = spec.owners[1];
              await wallet.confirmTransaction(transactionId, { from: confirmer });
              expect((await wallet.getConfirmationCount(transactionId)).toNumber()).to.eql(2);

              await wallet.revokeConfirmation(transactionId, { from: confirmer });
              expect((await wallet.getConfirmationCount(transactionId)).toNumber()).to.eql(1);
            });
          }
        ...
        describe('executeTransaction', async () => {
          ...
          const getTransactionCount = async (pending, executed) =>
            (await wallet.getTransactionCount(pending, executed)).toNumber();
          ...
          const getTransactionIds = async (pending, executed) => {
            ...
            const ids = await wallet.getTransactionIds(0, count, pending, executed);
          ...
          context('executed', async () => {
            ...
            beforeEach(async () => {
              ...
              expect(await wallet.isConfirmed(transactionId)).to.be.true();
              ...
              expect(web3.eth.getBalance(wallet.address).toNumber()).to.eql(initETHBalance - value);
              expect(web3.eth.getBalance(receiver).toNumber()).to.eql(receiverBalance.plus(value).toNumber());
            });
          ...
          context('not executed', async () => {
            ...
            beforeEach(async () => {
              ...
              expect(await getExecutedIds()).to.be.empty();
              expect(web3.eth.getBalance(wallet.address).toNumber()).to.eql(initETHBalance);
              expect(web3.eth.getBalance(receiver).toNumber()).to.eql(receiverBalance.toNumber());
            });
            ...
            context('not confirmed', async () => {
              ...
              it('should fail gracefully, if called by a confirmed owner', async () => {
                ...
                expect(web3.eth.getBalance(wallet.address).toNumber(), initETHBalance);
                expect(web3.eth.getBalance(receiver).toNumber(), receiverBalance.toNumber());
            ...
            context('confirmed', async () => {
              ...
              beforeEach(async () => {
                ...
                expect(web3.eth.getBalance(wallet.address).toNumber(), 0);
                ...
                expect(web3.eth.getBalance(wallet.address).toNumber()).to.eql(0);
                expect(web3.eth.getBalance(receiver).toNumber()).to.eql(receiverBalance.plus(walletBalance).toNumber());
                ...
                expect(web3.eth.getBalance(wallet.address).toNumber()).to.eql(value);
              });
          ...
          it(`should only send ${coin} when all confirmations were received`, async () => {
            ...
            for (let i = 1; i < spec.owners.length; i++) {
              ...
              if (confirmations < spec.requirement) {
                ...
                await wallet.confirmTransaction(transactionId, { from: confirmer });
                confirmations++;
                expect((await wallet.getConfirmationCount(transactionId)).toNumber()).to.eql(confirmations);
                ...
                if (confirmations === spec.requirement) {
                  ...
                  expect(walletBalance.toNumber()).to.eql(prevWalletBalance.minus(value).toNumber());
                  expect(receiverBalance.toNumber()).to.eql(prevReceiverBalance.plus(value).toNumber());
                } else {
                  expect(await wallet.isConfirmed(transactionId)).to.be.false();

                  expect(walletBalance.toNumber()).to.eql(prevWalletBalance.toNumber());
                  expect(receiverBalance.toNumber()).to.eql(prevReceiverBalance.toNumber());
                }
              } else {
                expect(await wallet.isConfirmed(transactionId)).to.be.true();
  ...
  describe('events', async () => {
    ...
    it('should emit events when confirming transaction', async () => {
      ...
      expect(event3.args.transactionId.toNumber()).to.eql(0);
    });
    ...
    it('should emit events when replacing owner', async () => {
      ...
      expect(event4.args.transactionId.toNumber()).to.eql(0);
    });
    ...
    it('should emit events when changing requirements', async () => {
      ...
      expect(event2.args.required.toNumber()).to.eql(2);
    ...
    it('should emit correct transaction IDs when submitting multiple transactions', async () => {
      ...
      expect(event3.args.transactionId.toNumber()).to.eql(1);

      const event4 = result.logs[1];
      expect(event4.event).to.eql('Confirmation');
      expect(event4.args.transactionId.toNumber()).to.eql(1);
      ...
      expect(event5.args.transactionId.toNumber()).to.eql(2);

      const event6 = result.logs[1];
      expect(event6.event).to.eql('Confirmation');
      expect(event6.args.transactionId.toNumber()).to.eql(2);
    });
  });
});
